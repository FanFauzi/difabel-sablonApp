                        ...
                        <div class="col-md-9">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    ...
                                    <p class="mb-1"><strong>Harga per Unit:</strong> Rp
                                        {{ number_format($order->product ? $order->product->price : 0, 0, ',', '.') }}</p>
                                </div>
                                <div class="col-sm-6">
                                    <p class="mb-1"><strong>Total Harga:</strong></p>
                                    <h4 class="text-success mb-1">Rp {{ number_format($order->total_price, 0, ',', '.') }}
                                    </h4>
                                </div>
                            </div>
                            <p class="mb-1"><strong>Tanggal Pesan:</strong> {{ $order->created_at->format('d M Y H:i') }}
                            </p>
                            <p class="mb-0"><strong>Terakhir Update:</strong>
                            ...
                            <div class="card bg-light">
                                <div class="card-body">
                                    @if ($order->design_size)
                                        <p class="mb-2"><strong>Ukuran Desain:</strong>
                                            {{ ucfirst($order->design_size) }}</p>
                                    @endif
                                    @if ($order->design_cost)
                                        <p class="mb-2"><strong>Biaya Desain:</strong> Rp
                                            {{ number_format($order->design_cost, 0, ',', '.') }}</p>
                                    @endif
                ...
                <div class="card-body">
                    ...
                    <div class="timeline">
                        <div class="timeline-item">
                            <div
                                class="timeline-marker {{ $order->status === 'pending' || $order->status === 'proses' || $order->status === 'selesai' ? 'bg-warning' : 'bg-secondary' }}">
                            </div>
                        ...
                        <div class="timeline-item">
                            <div
                                class="timeline-marker {{ $order->status === 'proses' || $order->status === 'selesai' ? 'bg-info' : 'bg-secondary' }}">
                            </div>
                            <div class="timeline-content">
                    ...
                    <div class="mt-4">
                        <a href="{{ $order->getWhatsAppUrl() }}" target="_blank" class="btn btn-success w-100 mb-2">
